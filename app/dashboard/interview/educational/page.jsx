...
const EducationalPage = () => {
  const { user } = useUser();
  const [educationalInterviews, setEducationalInterviews] = useState([]);
  ...
  const onStart = (id) => {
    router.push(`/dashboard/interview/educational/${id}/start`);
  };

  const onFeedbackPress = (id) => {
    router.push(`/dashboard/interview/educational/${id}/feedback`);
  };

  const onLive = (id) => {
    router.push(`/dashboard/interview/educational/${id}/live`);
  };

  const getEducationalInterviews = async (page = 0, size = 10) => {
    const email = user?.primaryEmailAddress?.emailAddress;

    try {
      const res = await fetch("/api/fetchEducationalInterviews", {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        throw new Error(
          data.message || "Failed to fetch educational interview data"
        );
      }

      const data = await res.json();

      setTotalCount(data.educationalInterviews.length);
      setEducationalInterviews(data.educationalInterviews);
    } catch (err) {
      console.error("Error fetching educational interviews:", err);
      toast.error(err.message || "Failed to fetch educational interviews");
    }
  };

  const handleAddEducationalInterview = async (e) => {
    e.preventDefault();
    setLoading(true);

    let inputPrompt = `Educational educationLevel: ${educationLevel}.
    ...
    try {
      await db.insert(EducationalInterview).values({
        educationLevel,
        resumeContent,
        educationalDocsContent: "NIL",

        interviewId: id,
      });

      setIsAddInterviewDialogOpen(false);
      const baseUrl = `/dashboard/interview/educational/${id}`;
      if (isLive) {
      ...
      console.error("Error adding educational interview:", error);
      toast.error("Failed to add educational interview.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDelete = async (id) => {
    try {
      ...
      if (!res) {
        throw new Error(
          res.message || "Failed to delete the educational interview"
        );
      }
  ...
  useEffect(() => {
    ...
    if (email) {
      getEducationalInterviews(pageIndex, pageSize);
    }
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="font-bold text-2xl">
              Create Your Educational Interview
            </DialogTitle>
          </DialogHeader>
          <DialogDescription>
            ...
            <form onSubmit={handleAddEducationalInterview}>
              <div className="my-3">
