...
import { Interview, JobInterview, MockInterview } from "@/utils/schema";
...
const StartInterview = ({ params }) => {
  ...
  const getInterview = async () => {
    try {
      ...
      console.error("Error fetching job interview:", err);
      toast.error(err.message || "Failed to fetch job interview");
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitInterview = async () => {
    try {
      ...
      setLoading(false);
      router.push(
        `/dashboard/interview/job/${interviewData?.interview?.id}/feedback`
      );
      return;
    } catch (err) {
      console.error(err);
