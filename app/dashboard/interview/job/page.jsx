...
const JobPage = () => {
  ...
  const getJobInterviews = async (page = 0, size = 10) => {
    const email = user?.primaryEmailAddress?.emailAddress;

    try {
      const res = await fetch("/api/fetchJobInterviews", {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        throw new Error(data.message || "Failed to fetch job interview data");
      }

      const data = await res.json();

      setTotalCount(data.jobInterviews.length);
      setJobInterviews(data.jobInterviews);
    } catch (err) {
      console.error("Error fetching job interviews:", err);
      toast.error(err.message || "Failed to fetch job interviews");
    }
  };

  const handleAddJobInterview = async (e) => {
    e.preventDefault();
    setLoading(true);

    let inputPrompt = `Job Position: ${jobPosition}, Job description: ${jobDescription}, Years of Experience: ${jobExperience}.
    ...
    try {
      await db.insert(JobInterview).values({
      ...
      const baseUrl = `/dashboard/interview/job/${id}/`;
      if (isLive) {
      ...
      console.error("Error adding job interview:", error);
      toast.error("Failed to add job interview.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDelete = async (id) => {
    try {
      ...
      if (!res) {
        throw new Error(res.message || "Failed to delete the job interview");
      }
  ...
  useEffect(() => {
    ...
    if (email) {
      getJobInterviews(pageIndex, pageSize);
    }
            ...
            {info.cell.row.original?.type === 0 ? (
              info.cell.row.original?.submitted ? (
                <Link
                  size="sm"
                  variant="muted"
                  href={`/dashboard/interview/job/${info.cell.row.original?.id}/feedback`}
                  className="mr-2 !p-0"
                  title="Feedback"
                >
                  <MessageSquareWarning />
                ...
                <Link
                  href={`/dashboard/interview/job/${info.cell.row.original?.id}/start`}
                  size="sm"
                  title="Start"
              ...
              >
                <Link
                  href={`/dashboard/interview/job/${info.cell.row.original?.id}/live`}
                >
                  <Video />
                </Link>
              </Button>
  ...
  return (
    <>
      <div className="flex gap-3 items-center mb-4">
        ...
        <span className="text-primaryColor-level2">/ Job Interview</span>
        <Button
          size="sm"
      ...
      >
        <DialogContent className="max-w-2xl">
          ...
          <DialogDescription>
            ...
            <form onSubmit={handleAddJobInterview}>
              <>
                <div className="my-3">
                ...
                <div className="my-3">
                  <label className="flex">Job Description / Tech Stack</label>
                  <Textarea
