...
function TechStackComponent({ isOpen }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const currentTechStacks = await db.select().from(TechStack).execute();

      // Create a map of the current IDs for easier comparison
      const currentTechStackIds = new Set(
        currentTechStacks.map((stack) => stack.id)
      );
      ...
      const idsToDelete = [...currentTechStackIds].filter(
        (id) => !newTechStackIds.has(id)
      );

      // Step 2: Find new records (IDs in the new state but not in DB)
      let newRecordsToInsert = techStacks.filter(
        (stack) => !currentTechStackIds.has(stack.id)
      );

      // Step 3: Find records that need to be updated (present in both the DB and the new state)
      const updatedRecords = techStacks.filter((stack) =>
        currentTechStackIds.has(stack.id)
      );
      ...
      if (newRecordsToInsert.length > 0) {
        ...
        await db.insert(TechStack).values(newRecordsToInsert).execute();
      }
